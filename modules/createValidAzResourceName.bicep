...
var regularName = replace(
  replace(
    replace(
      replace(
        replace(
          replace(namingConventionSegmentSeparatorProcessed, '{purpose}', toLower(purpose)),
          '{location}',
          location
        ),
        '{sequence}',
...
var shortName = replace(
  replace(
    replace(
      replace(
        replace(
          replace(namingConventionSegmentSeparatorProcessed, '{purpose}', toLower(take(purpose, 1))),
          '{location}',
          shortLocationValue
        ),
        '{sequence}',
